...
export const CheckInView: React.FC<CheckInViewProps> = ({
  ...
  // Auto tax calculations based on Tax Slab
    ...
    if (matched) {
      ...
      if (matched.items && matched.items.length > 0) {
        const itemsList = matched.items
          ...
          .join('\n\n');
        setDescription(
          `Order Details\nOrder ID: ${matched.id}${matched.status === 'Partial' ? ' (Partially Fulfilled)' : ''}\nSupplier: ${matched.supplier || 'N/A'}\nDate: ${orderDate}\nOrdered By: ${orderedBy}\n\n${itemsList}`
        );
      } else {
        setDescription(
          `Order Details\nOrder ID: ${matched.id}\nSupplier: ${matched.supplier || 'N/A'}\nDate: ${orderDate}\nOrdered By: ${orderedBy}`
  ...
  const handleFilesRead = async (files: FileList | File[]) => {
    ...
    try {
      ...
      toast.success(`Attached ${targetFiles.length} image(s) (Compressed for fast upload)`);
    } catch (err) {
          ...
          <form onSubmit={handleFormSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  disabled={disabled || !isTaxSlabSelected}
                  value={taxOption}
